...
assert.match(indexHtml, /4\.5\.17-adaptive-gate-rotation-nearby-hardening-20260909/, "the application build marker includes the 4.5.10 nearby direct progress hotfix");
assert.equal((indexHtml.match(/4\.5\.17-adaptive-gate-rotation-nearby-hardening-20260909/g) || []).length, 2, "Course and Practice use the same 4.5.10 build marker");
...
assert.match(indexHtml, /KNOWLEDGE_SCHEMA_VERSION=4/, "shared lexical knowledge has an explicit schema");
...
assert.match(indexHtml, /strength:0,sourceMode:"grammar"/, "grammar context is context evidence only and cannot raise lexical strength");
...
assert.match(serviceWorker, /languagedeck-4-5-17-adaptive-gate-rotation-nearby-hardening-20260909/, "service worker cache version is current");
...
assert.match(serviceWorker, /OPTIONAL_SHELL=\["\.\/vendor\/jsqr\.js"\]/, "the pinned QR decoder is opportunistically cached without breaking direct archives that have not vendored it yet");
assert.ok(!existsSync(join(root, "tests", "verify-maintenance-integrity-v3.mjs")), "obsolete always-red v3 maintenance test was removed");
